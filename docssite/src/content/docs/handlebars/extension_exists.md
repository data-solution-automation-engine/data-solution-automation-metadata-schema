---
title: '{{#exists}}'
description: Block helper that checks whether a specific metadata segment exists - optionally matching a given value within that segment.
---

This block helper extension specifically applies to the schema for data solution automation.

The helper targets specific segments in the metadata and evaluates whether the segment exists at all (without specifying an optional search value) or whether the provided optional search value exists in that segment.
