...
class Program
{
    static int Main(string[] args)
    {
        ...
        // Get the handlebars extensions from the DataSolutionAutomation class library.
    ...
    private static void RunAutomation(Options options, string inputFileName, string outputFileName = "")
    {
        try
        {
            ...
            if (options.output)
            {
                if (outputFileName == "")
                {
                    // Try to extract the name from the JSON metadata according to the Data Solution Automation schema
